import {WagYourBotWeb} from "./WagYourBotWeb";
...
export class StandaloneDiscordAPI {
    ...
    static async getGuildRoles(guildID: string, clientID: string, handler: WagYourBotWeb): Promise<{[key: string]: {name: string, color: string, position: number}}> {
        try {
            const token = await handler.database.getClientToken(clientID);
