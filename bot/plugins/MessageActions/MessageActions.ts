...
class InternalEmbedData extends CommandTree<MessageActionsData> {
    ...
    buildCommandTree(): void {
        this.then("--title")
            .then("contents", {type: /(.+?)(\s--|$)/, lookahead: true, argFilter: arg => <string>arg[1]}, async (args, remainingContent, member, guild, channel, message, handler) => {
            ...
            .or()
        .or("--description")
            .then("contents", {type: /([\s\S]+?)(\s--|$)/, lookahead: true, argFilter: arg => <string>arg[1]}, async (args, remainingContent, member, guild, channel, message, handler) => {
            ...
            .or()
        .or("--fieldTitle")
            .then("titleContents", {type: /(.+?)(\s--fieldDescription|$)/, argFilter: (arg) => <string>arg[1], lookahead: true})
                .then("--fieldDescription")
                    .then("descContents", {type: /([\s\S]+?)(\s--|$)/, lookahead: true, argFilter: arg => <string>arg[1]}, async (args, remainingContent, member, guild, channel, message, handler) => {
